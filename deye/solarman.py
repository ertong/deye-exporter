...
class Inverter:
    def __init__(self, path, serial, host, port, mb_slaveid, lookup_file):
        self._modbus: PySolarmanV5 = None
        self._serial = serial
        self.path = path
        self._host = host
        self._port = port
        ...
        if not self.lookup_file or lookup_file == 'parameters.yaml':
            self.lookup_file = '../inverter_definitions/deye_hybrid.yaml'
    ...
    def connect_to_server(self):
        if self._modbus:
            return self._modbus
        log.debug(f"Connecting to solarman data logger {self._host}:{self._port}")
    ...
    def disconnect_from_server(self):
        if self._modbus:
            try:
                log.debug(f"Disconnecting from solarman data logger {self._host}:{self._port}")
